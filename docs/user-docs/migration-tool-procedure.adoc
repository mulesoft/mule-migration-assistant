...
utility (packaged as an executable JAR file). After the Beta program,
MMA will be released as part of Studio. You will then
...
----
+
. For each application that belongs to that domain run MMA with the additional parameter `parentDomainBasePath` pointing to the unmigrated parent domain:
+
.Command-line Invocation
[source,console,linenums]
----
